...
import { addAccountsToList, createCRMList } from "../../../utils/supabase";
import { CombinedLinkedInAccount } from "../../../contexts/AccountContext";
import StyledMenuList from "../../../components/Custom/StyledMenuList";
...
const SaveAccounts: React.FC<SaveProfilesProps> = ({
  selectedProfiles,
  lists,
  onListsUpdate,
  account,
  ...
  const handleSaveToList = useCallback(
    async (list: List) => {
      ...
      setIsSaving(true);
      try {
        console.log("Selected Accounts", selectedProfiles);
        ...
        await addAccountsToList(list.id, profilesToSave);
  ...
  const handleCreateNewList = async () => {
    ...
    try {
      ...
      const newList = await createCRMList(newListName.trim(), {
          ...
          <ModalBody>
            <VStack spacing={4} align="stretch">
              {!selectedList ? (
                ...
                <Text color="gray.300">
                  Are you sure you want to save {selectedProfiles.length}{" "}
                  profile{selectedProfiles.length !== 1 ? "s" : ""} to this
                  list?
                </Text>
              )}
            </VStack>
