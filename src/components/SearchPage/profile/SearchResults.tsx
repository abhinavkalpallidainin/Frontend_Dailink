...
import ProfileSelectionControls from '../../Selection/ProfileSelectionControls';
import SaveProfiles from './SaveProfiles';
    ...
    <Box bg="gray.800" p={4} borderRadius="md" boxShadow="lg" w="full">
      <Flex justify="space-between" align="center" mb={4}>
        {isSearching ? (
          <HStack spacing={2}>
            <Spinner size="sm" />
            <Text color="white" fontSize="lg">Searching...</Text>
          </HStack>
        ) : (
        ...
        <Table variant="simple" size="sm">
          <Thead>
            <Tr>
              ...
              <Th color="gray.200">HEADLINE</Th>
              <Th color="gray.200">LOCATION</Th>
              <Th color="gray.200">NETWORK</Th>
          ...
          <Tbody>
            {searchResults.map((result) => (
              <Tr 
                key={result.id} 
                _hover={{ bg: "gray.700" }}
                bg={selectedProfiles.includes(result.id) ? "gray.700" : "transparent"}
              >
                <Td width="40px">
                  <Checkbox
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        ...
                        setSelectedProfiles(selectedProfiles.filter(id => id !== result.id));
                      }
                    }}
                    colorScheme="blue"
                  />
                ...
                <Td>
                  <VStack align="start" spacing={1}>
                    <Text color="white" fontWeight="bold">{result.name}</Text>
                    <Text color="gray.400" fontSize="sm">{result.public_identifier}</Text>
                  </VStack>
                </Td>
                <Td>
                ...
                <Td>
                  <Tooltip label={`${formatNetworkDistance(result.network_distance)} degree connection`}>
                    <Badge 
                      colorScheme={getNetworkBadgeColor(result.network_distance)}
                      fontSize="sm"
                    >
                      {formatNetworkDistance(result.network_distance)}
                    </Badge>
                  </Tooltip>
                </Td>
                <Td>
      ...
      {searchResults.length > 0 && (
        <Flex justify="center" align="center" mt={4} gap={4}>
          ...
          <HStack spacing={1}>
            ...
            <Text color="white" fontWeight="bold">{currentPage}</Text>
          </HStack>
          <IconButton
