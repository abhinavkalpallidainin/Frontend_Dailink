...
const CampaignDashboard: React.FC = () => {
  ...
  const fetchCampaigns = useCallback(async () => {
    if (!selectedAccount) {
      setIsLoading(false);
      setError(
        "No account selected. Please select an account to manage campaigns."
      );
      return;
    }
  ...
  const handleCreateCampaign = async () => {
    if (!selectedAccount || !selectedAccount.id) {
      toast({
        title: "Error",
        description:
          "No account selected. Please select an account to create a campaign.",
        status: "error",
        duration: 3000,
        isClosable: true,
      });
  ...
  const handleStatusChange = async (campaign: Campaign) => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "There was an error updating the campaign status. Please try again.",
        status: "error",
        duration: 5000,
        isClosable: true,
      });
    ...
    return (
      <Box bg={bgColor} minHeight="100vh" p={8}>
        <Alert status="warning">
          <AlertIcon />
          No LinkedIn account selected. Please select an account to manage
          campaigns.
        </Alert>
      </Box>
    );
      ...
      >
        ...
        <Flex direction="column" mb={8}>
          <Heading size="xl" color={textColor} mb={2}>
            Campaigns for {selectedAccount.name}
          </Heading>
          <Text fontSize="md" color="gray.400">
            Last 24 hrs' actions: {actionStats.last24Hours} of{" "}
            {actionStats.total}
          </Text>
        </Flex>
        ...
        <SimpleGrid columns={{ base: 1, md: 2, lg: 4 }} spacing={6} mb={8}>
          ...
          <Stat bg={cardBgColor} p={4} borderRadius="md" boxShadow="md">
            ...
            <StatNumber color={textColor}>
              {campaigns.reduce(
                (sum, campaign) => sum + campaign.successful,
                0
              )}
            </StatNumber>
          </Stat>
            ...
            <StatNumber color={textColor}>
              {(
                (campaigns.reduce(
                  ...
                  campaigns.reduce(
                    (sum, campaign) => sum + campaign.contacts,
                    0
                  )) *
                100
              ).toFixed(1)}
        ...
        <Tabs variant="soft-rounded" colorScheme="blue" mb={8}>
          <TabList>
            <Tab
              color={textColor}
              _selected={{ color: "blue.300", bg: "blue.800" }}
            >
              All Campaigns
            </Tab>
            <Tab
          ...
          <TabPanels>
            ...
            <TabPanel>
              <CampaignTable
                campaigns={filteredCampaigns.filter(
                  (c) => c.status === "Running"
                )}
                onEdit={handleEditCampaign}
                onDelete={handleDeleteCampaign}
          ...
          <ModalBody>
            ...
            <FormControl mt={4}>
              ...
              >
                <option
                  value="Outreach"
                  style={{ backgroundColor: "gray.700" }}
                >
                  Outreach
                </option>
                <option
        ...
        <Tbody>
          {campaigns.map((campaign) => (
            <Tr key={campaign.id} _hover={{ bg: bgColor }}>
              ...
              <Td>
                <Badge
                  colorScheme={campaign.status === "Running" ? "green" : "red"}
                >
                  {campaign.status}
                </Badge>
              </Td>
              <Td>
                <Tooltip
                  label={`${campaign.successful} / ${campaign.contacts}`}
                >
                  <Progress
                    value={
                      (campaign.successful / (campaign.contacts || 1)) * 100
                    }
                    size="sm"
                    colorScheme="blue"
                  />
              ...
              <Td color={textColor}>
                {(
                  (campaign.successful / (campaign.contacts || 1)) *
                  100
                ).toFixed(1)}
                %
              </Td>
                ...
                <HStack spacing={2}>
                  ...
                  <Menu>
                    ...
                    <MenuList>
                      ...
                      <MenuItem onClick={() => onEdit(campaign.id)}>
                        Edit
                      </MenuItem>
                      <MenuItem onClick={() => onDelete(campaign.id)}>
                        Delete
                      </MenuItem>
                    </MenuList>
                  </Menu>
